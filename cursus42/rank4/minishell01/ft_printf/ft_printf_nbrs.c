...
/*   Updated: 2019/12/10 11:58:07 by acarvaja         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	do_num_dandi(t_datatype *data, va_list ap)
{
	...
	if (data->datanbr || !data->fprecision || data->val_precission > 0)
...
void	do_num_u(t_datatype *data, va_list ap)
{
	...
	data->fuint = 1;
	if ((!data->fprecision || data->dataunbr || data->val_precission > 0))
		data->result += ft_uintlen(data->dataunbr);
}

void	do_num_exhl(t_datatype *data, va_list ap)
{
	data->dataexh = va_arg(ap, unsigned int);
	if ((!data->fprecision || data->dataexh || data->val_precission > 0))
		data->result += ft_strlen(ft_atoa_lowexh(data->dataexh));
}

void	do_num_exhu(t_datatype *data, va_list ap)
{
	data->dataupexh = va_arg(ap, unsigned int);
	if ((!data->fprecision || data->dataupexh || data->val_precission > 0))
